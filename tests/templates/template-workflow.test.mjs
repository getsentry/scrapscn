...
test("a scaffolded template is discovered without a registry edit", async () => {
  await withTemplates(async (root) => {
    ...
    assert.deepEqual(templates.map(({ metadata }) => metadata.slug), ["issue-review"])
    assert.equal(templates[0].metadata.title, "Issue Review")
    assert.match(await readFile(path.join(root, "issue-review", "template.tsx"), "utf8"), /SentryPageFrame/)
  })
})
...
test("discovery rejects invalid metadata and missing implementations", async () => {
  await withTemplates(async (root) => {
    ...
    await writeFile(path.join(root, "broken", "template.json"), JSON.stringify({ slug: "broken", title: "Broken", description: "Broken template", tags: [], extra: true }))
...
test("the scaffold refuses invalid slugs and existing targets", async () => {
  await withTemplates(async (root) => {
    await assert.rejects(scaffoldTemplate({ slug: "Not Valid", templatesRoot: root }), /lowercase kebab case/)
    await scaffoldTemplate({ slug: "existing", templatesRoot: root })
    await assert.rejects(scaffoldTemplate({ slug: "existing", templatesRoot: root }), /exist|not empty/i)
  })
})
