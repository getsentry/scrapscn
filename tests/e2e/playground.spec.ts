...
test("configures, shares, and restores the regular Scraps Link workbench", async ({ browser, page }) => {
  ...
  await expect(preview.getByRole("link", { name: "Read the docs" })).toHaveAttribute("target", "_blank")
  ...
  await expect(restored.getByTestId("link-preview").getByText("Shared issue")).not.toHaveAttribute("href")
  ...
  await page.goBack()
  await expect(page.locator('[data-slot="playground-canvas"]')).toHaveAttribute("data-component", "link")
  await expect(page.getByLabel("Link label")).toHaveValue("Shared issue")
  await expect(page.getByLabel("Disabled link")).toBeChecked()
  await expect(page.getByLabel("Preview width")).toHaveValue("mobile")
  ...
  await expect(page.getByTestId("link-preview").getByRole("link", { name: "Open issue details" })).toHaveAttribute("href", "/issues/")
...
test("configures, responds, shares, restores, and resets the Text workbench", async ({ page }) => {
  ...
  await expect(page.locator('[data-slot="playground-canvas"]')).toHaveAttribute("data-component", "text")
  ...
  await expect(page.getByTestId("text-inherit-class-preview")).toHaveCSS("color", "rgb(0, 87, 184)")
  await expect(page.getByTestId("heading-inherit-style-preview")).toHaveCSS("color", "rgb(180, 35, 24)")
  ...
  expect(await page.evaluate(async () => (await document.fonts.load('425 14px "Roboto Mono Variable"')).length)).toBeGreaterThan(0)
  ...
  expect((await configuredText.evaluate((element) => getComputedStyle(element).fontFamily))).toContain("Roboto Mono")
  expect((await configuredText.evaluate((element) => getComputedStyle(element).textDecorationLine)).split(" ").sort()).toEqual(["line-through", "underline"])
  ...
  await expect(rawInlineCode).toHaveCSS("font-weight", "425")
