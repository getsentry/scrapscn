...
test("rejects an incorrectly mapped Checkbox checked state", () => {
  const results = previews();
  const checked = results.find((result) =>
    result.propertyCombinationLabel.includes("checked=True, state=Default"),
  );
...
test("rejects an incorrectly mapped Checkbox disabled state", () => {
  const results = previews();
  const disabled = results.find((result) =>
    result.propertyCombinationLabel.includes("state=disabled"),
  );
...
test("rejects a public prop on a design-only interaction state", () => {
  ...
  const focused = results.find((result) =>
    result.propertyCombinationLabel.includes("state=Focused"),
  );
