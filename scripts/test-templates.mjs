...
for (const { directory, metadata } of templates) {
  ...
  if (
    !generatedManifest.includes(`./${directory}/template`) ||
    !generatedManifest.includes(`\"slug\":${JSON.stringify(metadata.slug)}`)
  ) {
...
process.stdout.write(
  `Validated ${templates.length} template${templates.length === 1 ? "" : "s"}.\n`,
);
