...
const expectedKeys = SIZES.flatMap((size) =>
  CHECKED_VALUES.flatMap((checked) => STATES.map((state) => `${size}|${checked}|${state}`)),
);
...
function combinationKey(label) {
  ...
  if (propertyNames.join("|") !== "checked|size|state") {
    throw new Error(
      `Figma Checkbox schema contains unexpected properties: ${propertyNames.join(", ")}`,
    );
  }
  ...
  function bindsCheckbox(name) {
    ...
    if (ts.isObjectBindingPattern(name) || ts.isArrayBindingPattern(name)) {
      return name.elements.some(
        (element) => ts.isBindingElement(element) && bindsCheckbox(element.name),
      );
    }
    return false;
  }
...
export function validateCheckboxPreviews(previews) {
  ...
  if (missing.length || unexpected.length) {
    throw new Error(
      `Figma Checkbox schema mismatch. Missing: ${missing.join(", ") || "none"}. Unexpected: ${unexpected.join(", ") || "none"}.`,
    );
  }
  ...
  for (const preview of allPreviews) {
    ...
    if (
      ...
      throw new Error(
        `Figma Checkbox preview used the wrong component for ${preview.propertyCombinationLabel}`,
      );
    }

    const [size, checked, state] = combinationKey(preview.propertyCombinationLabel).split("|");
    const expectedChecked =
      checked === "True" ? true : checked === "indeterminate" ? "indeterminate" : undefined;
    const expectedDisabled = state === "disabled" ? true : undefined;
    const matchesProp = (name, expected) =>
      expected === undefined
        ? !snippet.checkboxProps.has(name)
        : snippet.checkboxProps.get(name) === expected;
    if (
      ...
      throw new Error(
        `Figma Checkbox preview mapped the wrong props for ${preview.propertyCombinationLabel}`,
      );
    }
  }
