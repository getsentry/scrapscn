...
export function CheckboxWorkbench({
  ...
  const [state, setState] = useState(() =>
    parseCheckboxWorkbench(new URLSearchParams(sourceSearch)),
  );
  ...
  const checkboxValue =
    state.checked === "indeterminate" ? "indeterminate" : state.checked === "true";
  const controls = (
      ...
      <div className="grid gap-4">
        ...
        <label className="grid gap-2 text-base font-medium sm:text-sm">
          ...
          <select
            ...
            name="checkbox-size"
            value={state.size}
            onChange={(event) => update({ ...state, size: parseCheckboxSize(event.target.value) })}
          >
        ...
        <label className="grid gap-2 text-base font-medium sm:text-sm">
          ...
          <select
            ...
            value={state.checked}
            onChange={(event) =>
              update({ ...state, checked: parseCheckedState(event.target.value) })
            }
          >
      ...
      <div className="grid content-start gap-2 sm:border-l sm:border-foreground/10 sm:pl-4">
        <div className="flex items-center justify-between gap-2">
          ...
          <Button
            ...
            disabled={state.items.length === defaultNotificationIds.length}
            icon={<Plus aria-hidden="true" className="size-4 shrink-0" />}
            type="button"
            variant="transparent"
            onClick={() => {
              const item = defaultNotificationIds.find((id) => !state.items.includes(id));
              if (item) updateItems([...state.items, item]);
            }}
          />
        </div>
        <ol className="grid gap-1">
          {state.items.map((item, index) => {
            ...
            return (
              <li
                className="flex min-w-0 items-center gap-1 rounded-md border border-foreground/10 px-2 py-1"
                key={item}
              >
                ...
                <Button
                  ...
                  className="size-11 sm:size-10"
                  disabled={index === 0}
                  icon={<ArrowUp aria-hidden="true" className="size-3.5 shrink-0" />}
                  type="button"
                  variant="transparent"
                ...
                <Button
                  ...
                  icon={<ArrowDown aria-hidden="true" className="size-3.5 shrink-0" />}
                  type="button"
                  variant="transparent"
                  onClick={() => moveItem(index, 1)}
                />
                <Button
                  ...
                  icon={<Trash2 aria-hidden="true" className="size-3.5 shrink-0" />}
                  type="button"
                  variant="transparent"
                  onClick={() => updateItems(state.items.filter((id) => id !== item))}
                />
              </li>
            );
          })}
  ...
  const preview = (
    <div className="grid max-w-3xl gap-8">
      <Container
        background="primary"
        border="primary"
        data-testid="layout-separator-proof"
        padding="md"
        radius="md"
      >
        <Stack direction={{ zero: "column", "screen:lg": "row" }} gap="sm">
            ...
            <PlaygroundSlot.Outlet name="utility">
              {(props, hasConsumers) => (
                <div
                  ...
                  data-testid="slot-playground-outlet"
                >
      ...
      <form
        ...
        onSubmit={(event) => {
          ...
          const selectedItems = state.items.filter((item) => data.has(item));
          const saved = selectedItems
            .map((item) => getNotificationLabel(item, state.label))
            .join(", ");
          setSubmittedState(saved ? `Saved: ${saved}` : "Saved: no email notifications");
        }}
      >
        <div className="grid gap-1 border-b border-foreground/10 pb-4">
          <h2 className="text-lg font-semibold" id="email-heading">
            Email notifications
          </h2>
          <p className="text-base text-pretty text-muted-foreground sm:text-sm">
            Control the messages sent to project members.
          </p>
        </div>
            ...
            <div className="flex items-start gap-3 border-b border-foreground/10 py-4" key={item}>
              <span className="flex h-lh items-center text-lg sm:text-sm">
                <Checkbox
                  ...
                  checked={item === "new-issues" ? checkboxValue : state.selected.includes(item)}
                  ...
                  onChange={(event) => {
                    if (item === "new-issues") {
                      update({ ...state, checked: event.target.checked ? "true" : "false" });
                      return;
                    }
              ...
              <div className="min-w-0">
                <label className="cursor-pointer font-medium" htmlFor={checkboxId} id={labelId}>
                  {itemLabel}
                </label>
                <p className="mt-1 text-base text-muted-foreground sm:text-sm" id={descriptionId}>
                  {content.description}
                </p>
              </div>
            </div>
  ...
  return children({
    ...
    reset: () => {
      ...
      return serializeCheckboxWorkbench(next);
    },
