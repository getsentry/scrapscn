...
function parseMaximum(value: string | null): SplitPanelMaximum {
  ...
  return Number.isFinite(maximum)
    ? { kind: "fixed", value: Math.max(0, Math.round(maximum)) }
    : { kind: "container" };
}
...
export function serializeSplitPanelWorkbench(state: SplitPanelWorkbenchState) {
  ...
  params.set(
    "splitPanelMaxSize",
    state.maximum.kind === "fixed" ? String(state.maximum.value) : "container",
  );
...
export function SplitPanelWorkbench({ children, onSearchChange, sourceSearch }: WorkbenchProps) {
  const [state, setState] = useState(() =>
    parseSplitPanelWorkbench(new URLSearchParams(sourceSearch)),
  );
  ...
  const controls = (
    <div className="grid gap-4">
      <div>
        <h2 className="text-sm font-semibold">Split Panel setup</h2>
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        ...
        <label className="grid gap-2 text-base font-medium sm:text-sm">
          ...
          <select
            ...
            onChange={(event) =>
              update({ ...state, placement: parsePlacement(event.target.value) })
            }
          >
        ...
        <label className="flex min-h-11 items-center gap-3 text-base font-medium sm:min-h-10 sm:text-sm">
          <Checkbox
            checked={state.hasFill}
            name="split-panel-fill"
            onChange={(event) => update({ ...state, hasFill: event.target.checked })}
          />
          Include fill pane
        </label>
        <label className="grid gap-2 text-base font-medium sm:text-sm">
          ...
          <input
            ...
            onChange={(event) =>
              update({ ...state, defaultSize: parseNumber(event.target.value, 200) })
            }
          />
        </label>
        <label className="grid gap-2 text-base font-medium sm:text-sm">
          Initial size
          <input
            aria-label="Initial size"
            className={controlClassName}
            min={0}
            type="number"
            value={state.initialSize}
            onChange={(event) => {
              const initialSize = parseNumber(event.target.value, 200);
              update({ ...state, initialSize });
              panelRef.current?.setSize(initialSize, true);
            }}
          />
        </label>
        <label className="grid gap-2 text-base font-medium sm:text-sm">
          ...
          <input
            ...
            value={state.minSize}
            onChange={(event) =>
              update({ ...state, minSize: parseNumber(event.target.value, 100) })
            }
          />
        </label>
        <label className="grid gap-2 text-base font-medium sm:text-sm">
          Fill minimum
          <input
            aria-label="Fill minimum"
            className={controlClassName}
            min={0}
            type="number"
            value={state.fillMinSize}
            onChange={(event) =>
              update({ ...state, fillMinSize: parseNumber(event.target.value, 120) })
            }
          />
        </label>
        <label className="grid gap-2 text-base font-medium sm:text-sm">
          ...
          <label className="grid gap-2 text-base font-medium sm:text-sm">
            ...
            <input
              ...
              onChange={(event) =>
                update({
                  ...state,
                  maximum: { kind: "fixed", value: parseNumber(event.target.value, 400) },
                })
              }
            />
          </label>
    ...
    <Container background="primary" border="primary" minWidth="0" padding="md" radius="md">
      <Stack gap="sm">
        <div className="flex items-center justify-between gap-3">
          <span className="text-sm font-medium">Split panel proof</span>
          <Button
            size="sm"
            type="button"
            variant="secondary"
            onClick={() => panelRef.current?.setSize(state.defaultSize, true)}
          >
            Set default size
          </Button>
        </div>
        <div
          className="h-[320px] min-w-0 overflow-hidden rounded-md border border-foreground/10"
          data-testid="split-panel-frame"
        >
          <SplitPanel
            ref={panelRef}
            ...
            fill={
              state.hasFill ? (
                <div
                  className="size-full p-4 text-sm text-muted-foreground"
                  data-testid="split-panel-fill"
                >
                  Fill pane
                </div>
              ) : undefined
            ...
            sized={
              <div
                className="size-full bg-muted p-4 text-sm font-medium"
                data-testid="split-panel-sized"
              >
                Sized pane
              </div>
            }
  ...
  return children({
    ...
    reset: () => {
      ...
      return serializeSplitPanelWorkbench(next);
    },
