...
function defaults(): LoaderState {
  return { variant: "vibrant", width: "240" };
}
...
export function LoaderWorkbench({ children, onSearchChange, sourceSearch }: WorkbenchProps) {
  ...
  function update(next: LoaderState) {
    setState(next);
  ...
  const controls = (
    <div className="grid gap-4">
      <div>
        <h2 className="text-sm font-semibold">Loader setup</h2>
      ...
      <label className="grid gap-1 text-sm">
        ...
        <select
          ...
          onChange={(event) =>
            update({
              ...state,
              variant: event.target.value === "monochrome" ? "monochrome" : "vibrant",
            })
          }
        >
  ...
  const preview = (
    <div
      ...
      style={{ boxSizing: "border-box", minHeight: 176, padding: 24, width: "100%" }}
    >
      <div style={{ color: state.variant === "monochrome" ? "#7553ff" : undefined, width }}>
        <IndeterminateLoader variant={state.variant} />
      </div>
    </div>
  );
  return children({
    breadcrumbs: ["Components", "Loader"],
    controls,
    description: "Test the regular Scraps indeterminate squiggle and its width-sensitive timing.",
    preview,
    reset: () => {
      const next = defaults();
      setState(next);
