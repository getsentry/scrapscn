...
function panel(props: Partial<React.ComponentProps<typeof SplitPanel>> = {}) {
  return (
    <SplitPanel defaultSize={200} fill={<div>fill</div>} sized={<div>sized</div>} {...props} />
  );
}
...
describe("SplitPanel", () => {
  it("renders both panes and a divider", async () => {
    ...
    expect(view.textContent).toContain("sized");
    expect(view.textContent).toContain("fill");
    expect(separator(view)).toBeTruthy();
  });
  it("renders one pane without fill", async () => {
    const view = await render(panel({ fill: undefined }));
    expect(view.textContent).toContain("sized");
    expect(view.querySelector('[role="separator"]')).toBeNull();
  });
  it("floors a seeded size", async () => {
    const view = await render(panel({ initialSize: -50, minSize: 100 }));
    expect(separator(view).getAttribute("aria-valuenow")).toBe("100");
  });
  it("preserves the sized pane when fill changes", async () => {
    const sized = <div>sized</div>;
    const view = await render(
      <SplitPanel defaultSize={200} fill={<div>fill</div>} sized={sized} />,
    );
    const before = [...view.querySelectorAll("div")].find(
      (element) => element.children.length === 0 && element.textContent === "sized",
    );
    ...
    const after = [...view.querySelectorAll("div")].find(
      (element) => element.children.length === 0 && element.textContent === "sized",
    );
    expect(after).toBe(before);
  });
  it("orders end placement after fill", async () => {
    const view = await render(panel({ placement: "end" }));
    const text = view.textContent ?? "";
    expect(text.indexOf("fill")).toBeLessThan(text.indexOf("sized"));
  });
  it("sets separator semantics", async () => {
    const view = await render(panel({ minSize: 100, maxSize: 600 }));
  ...
  it("supports imperative setSize", async () => {
    ...
    await act(async () => ref.current?.setSize(350));
  ...
  it("derives max from the container", async () => {
    ...
    const view = await render(panel({ fillMinSize: 400, minSize: 100 }));
    expect(separator(view).getAttribute("aria-valuemax")).toBe("199");
  });
  it("does not clamp max below min", async () => {
    vi.spyOn(HTMLElement.prototype, "clientWidth", "get").mockReturnValue(50);
    const view = await render(panel({ minSize: 100 }));
    expect(separator(view).getAttribute("aria-valuemax")).toBe("100");
  });
  it("double click restores default", async () => {
    const onResizeEnd = vi.fn();
    const view = await render(panel({ initialSize: 400, minSize: 100, onResizeEnd }));
    await act(async () =>
      separator(view).dispatchEvent(new MouseEvent("dblclick", { bubbles: true })),
    );
    expect(separator(view).getAttribute("aria-valuenow")).toBe("200");
    expect(onResizeEnd).toHaveBeenCalled();
  });
  it("reports clamped resize start", async () => {
    const onResizeEnd = vi.fn();
    const view = await render(panel({ initialSize: -50, minSize: 100, onResizeEnd }));
    await act(async () =>
      separator(view).dispatchEvent(new MouseEvent("dblclick", { bubbles: true })),
    );
  ...
  it("uses arrows from the visible size", async () => {
    const view = await render(panel({ initialSize: -50, minSize: 100 }));
    await act(async () =>
      separator(view).dispatchEvent(
        new KeyboardEvent("keydown", { bubbles: true, key: "ArrowRight" }),
      ),
    );
    expect(separator(view).getAttribute("aria-valuenow")).toBe("110");
  });
  it("reports a clamped mount resize", async () => {
    const onResize = vi.fn();
    await render(panel({ initialSize: -50, minSize: 100, onResize }));
    expect(onResize).toHaveBeenCalledWith(100);
  });
  it("ignores unbounded Home", async () => {
    const onResizeEnd = vi.fn();
    const view = await render(panel({ placement: "end", minSize: 100, onResizeEnd }));
    await act(async () =>
      separator(view).dispatchEvent(new KeyboardEvent("keydown", { bubbles: true, key: "Home" })),
    );
    expect(onResizeEnd).not.toHaveBeenCalled();
  });
  it("reports keyboard resize end", async () => {
    const onResizeEnd = vi.fn();
  ...
  it("inverts arrows for end placement", async () => {
    const view = await render(panel({ placement: "end" }));
    await act(async () =>
      separator(view).dispatchEvent(
        new KeyboardEvent("keydown", { bubbles: true, key: "ArrowRight" }),
  ...
  it("places Home and End at the correct bounds", async () => {
    ...
    await act(async () =>
      separator(start).dispatchEvent(new KeyboardEvent("keydown", { bubbles: true, key: "End" })),
    );
    ...
    await act(async () =>
      separator(end).dispatchEvent(new KeyboardEvent("keydown", { bubbles: true, key: "End" })),
    );
  ...
  it("reseeds the initial size when placement changes the drawer direction", async () => {
    function Harness() {
      ...
      return (
        <>
          <button onClick={() => setPlacement("end")}>Flip</button>
          {panel({ initialSize: 180, placement })}
        </>
      );
    }
  ...
  it("reseeds the initial size when a responsive orientation changes direction", async () => {
    ...
    await act(async () =>
      verticalSeparator.dispatchEvent(
