...
export const VisualAcceptanceMatrix: Story = {
  render: () => (
    <div className="grid gap-6">
      {(["light", "dark"] as const).map((theme) => (
        <section key={theme} className={theme === "dark" ? "dark" : undefined} aria-labelledby={`${theme}-matrix-heading`}>
          <div className="grid gap-4 rounded-lg border border-foreground/10 bg-background p-5 text-foreground">
            <h2 id={`${theme}-matrix-heading`} className="font-semibold capitalize">{theme}</h2>
            {[false, true].map((disabled) => (
              <div key={String(disabled)} className="grid grid-cols-3 items-center gap-4">
                {checkboxSizes.flatMap((size) =>
                  checkboxStates.map((checked) => {
                    const label = `${theme} ${size} ${String(checked)} ${disabled ? "disabled" : "enabled"}`
                    return (
                      <label key={label} className="flex items-center gap-2 text-sm">
                        <Checkbox name={label} size={size} checked={checked} disabled={disabled} onChange={() => {}} />
                        <span>{label}</span>
                      </label>
                    )
                  })
  ...
  play: async ({ canvasElement }) => {
    ...
    const lightXs = canvas.getByRole("checkbox", { name: "light xs true enabled" })
    ...
    await expect(lightIcon?.querySelector("path")?.getAttribute("d")).toBe("M2.86 9.14C4.42 10.7 6.9 13.14 6.86 13.14L12.57 3.43")
    await expect(lightIcon?.getAttribute("stroke-width")).toBe("1.8")

    const darkMd = canvas.getByRole("checkbox", { name: "dark md indeterminate enabled" })
  ...
  render: function ControlledCheckboxStory() {
    ...
    return (
      <label className="flex cursor-pointer items-center gap-2">
        <Checkbox name="controlled" checked={checked} onChange={(event) => setChecked(event.target.checked)} />
...
export const Disabled: Story = {
  render: () => (
    <div className="flex items-center gap-5">
      ...
      <Checkbox name="disabled-unchecked" checked={false} disabled readOnly aria-label="Unchecked disabled" />
    </div>
  ),
}
...
export const InteractionContracts: Story = {
  render: () => (
    <div className="grid gap-4">
      <Checkbox checked={false} onChange={() => {}} aria-label="Interactive" style={{ color: "rgb(200, 0, 0)" }} />
      <Checkbox checked disabled aria-label="Disabled contract" />
      <Checkbox checked readOnly aria-label="Read-only contract" />
      <Checkbox checked aria-disabled="true" onChange={() => {}} aria-label="ARIA-disabled contract" />
    </div>
  ),
  play: async ({ canvasElement }) => {
    const canvas = within(canvasElement)
    const interactive = canvas.getByRole("checkbox", { name: "Interactive" })
    const interactionLayer = interactive.parentElement?.querySelector('[role="presentation"]') as HTMLElement
    await expect(getComputedStyle(interactionLayer).backgroundColor).toBe("rgb(200, 0, 0)")

    await userEvent.tab()
    await expect(interactive).toHaveFocus()
    await expect(getComputedStyle(interactive.nextElementSibling as HTMLElement).boxShadow).toContain("rgb(117, 83, 255) 0px 0px 0px 2px")

    const disabled = canvas.getByRole("checkbox", { name: "Disabled contract" })
    ...
    const readOnly = canvas.getByRole("checkbox", { name: "Read-only contract" })
    ...
    const ariaDisabled = canvas.getByRole("checkbox", { name: "ARIA-disabled contract" })
    await expect(getComputedStyle(ariaDisabled.nextElementSibling as HTMLElement).opacity).toBe("0.6")
