...
export const States: Story = {
  render: () => (
    <div className="grid grid-cols-2 gap-5">
      ...
      <div role="listbox" aria-label="Selected interaction list">
        <div className={interactionTargetClassName} role="option" aria-label="Selected interaction" aria-selected="true">
          <InteractionStateLayer />
        </div>
      </div>
      <button className={interactionTargetClassName} aria-label="Expanded interaction" aria-expanded="true">
        <InteractionStateLayer />
      </button>
      <div role="listbox" aria-label="Selected without background list">
        <div className={interactionTargetClassName} role="option" aria-label="Selected without background" aria-selected="true">
      ...
      <button className={interactionTargetClassName} aria-label="ARIA disabled interaction" aria-disabled="true">
        <InteractionStateLayer isPressed />
      </button>
      <div role="listbox" aria-label="Selected and disabled list">
        <div className={interactionTargetClassName} role="option" aria-label="Selected and disabled" aria-disabled="true" aria-selected="true">
          <InteractionStateLayer />
        </div>
      </div>
      <button className={interactionTargetClassName} aria-label="Expanded and disabled" aria-disabled="true" aria-expanded="true">
        <InteractionStateLayer />
      </button>
  ...
  play: async ({ canvasElement }) => {
    ...
    await expect(layerFor("Uncontrolled interaction")).toHaveAttribute("data-is-hovered", "undefined")
    await expect(layerFor("Uncontrolled interaction")).toHaveAttribute("data-is-pressed", "undefined")
...
export const DomContract: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    await expect(knownTokenLayer.style.color).toBe("var(--scraps-theme-primary, var(--primary, primary))")
    await expect(semanticTokenLayer.style.color).toBe("var(--scraps-theme-warning, var(--warning, warning))")
    await expect(legacyTokenLayer.style.color).toBe("var(--scraps-theme-blue500, var(--blue500, blue500))")
